...
const RiskBadge = ({ item }) => {
  ...
  if (isCritical || is_critical) return <span className="inline-flex px-3 py-1 rounded-full text-[11px] font-bold bg-purple-100 text-purple-700 border border-purple-200 items-center gap-1.5 shadow-sm whitespace-nowrap"><Siren className="w-3.5 h-3.5"/> 重大</span>;
  if (risk === '高' || risk === 'High') return <span className="inline-flex px-3 py-1 rounded-full text-[11px] font-bold bg-red-100 text-red-700 border border-red-200 whitespace-nowrap">高</span>;
  if (risk === '中' || risk === 'Medium') return <span className="inline-flex px-3 py-1 rounded-full text-[11px] font-bold bg-amber-100 text-amber-700 border border-amber-200 whitespace-nowrap">中</span>;
  return <span className="inline-flex px-3 py-1 rounded-full text-[11px] font-bold bg-emerald-100 text-emerald-700 border border-emerald-200 whitespace-nowrap">低</span>;
};
  ...
  return (
    <div className="h-full flex flex-col animate-in fade-in duration-500">
      ...
      <div className="flex gap-3 mb-6 overflow-x-auto pb-2 px-1">
        ...
        <button onClick={() => setFilter('critical')} className={`px-6 py-3 rounded-2xl text-sm font-bold whitespace-nowrap transition-all flex items-center gap-2 ${filter === 'critical' ? 'bg-red-600 text-white shadow-lg shadow-red-200 scale-105' : 'bg-white text-slate-500 border border-slate-100 hover:bg-red-50 hover:text-red-600'}`}>
          <Siren className="w-4 h-4"/> 重大・高 <span className="ml-1 opacity-70 text-xs font-normal">({counts.critical})</span>
        </button>
        <button onClick={() => setFilter('medium')} className={`px-6 py-3 rounded-2xl text-sm font-bold whitespace-nowrap transition-all ${filter === 'medium' ? 'bg-amber-500 text-white shadow-lg shadow-amber-200 scale-105' : 'bg-white text-slate-500 border border-slate-100 hover:bg-amber-50 hover:text-amber-600'}`}>
          中リスク <span className="ml-1 opacity-70 text-xs font-normal">({counts.medium})</span>
        </button>
        <button onClick={() => setFilter('low')} className={`px-6 py-3 rounded-2xl text-sm font-bold whitespace-nowrap transition-all ${filter === 'low' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-200 scale-105' : 'bg-white text-slate-500 border border-slate-100 hover:bg-emerald-50 hover:text-emerald-600'}`}>
        ...
        <div className="flex-1 overflow-y-auto">
           <table className="w-full text-left border-collapse">
             <thead className="bg-slate-50/90 backdrop-blur sticky top-0 z-10 border-b border-slate-200 shadow-sm">
               <tr>
                 <th className="px-8 py-5 text-xs font-bold text-slate-400 uppercase tracking-wider w-40 text-center">判定結果</th>
                 <th className="px-8 py-5 text-xs font-bold text-slate-400 uppercase tracking-wider w-32 text-center">商品画像</th>
                 <th className="px-8 py-5 text-xs font-bold text-slate-400 uppercase tracking-wider min-w-[320px]">商品詳細</th>
                 <th className="px-8 py-5 text-xs font-bold text-slate-400 uppercase tracking-wider w-1/3 min-w-[400px]">AI弁理士の分析</th>
                 <th className="px-8 py-5 text-xs font-bold text-slate-400 uppercase tracking-wider w-32 text-center">ソース</th>
               </tr>
             </thead>
...
const DashboardView = ({ sessions, onNavigate, onResume, onForceStop, onInspectSession }) => {
  ...
  const stats = useMemo(() => {
    let totalChecks = 0; let totalCritical = 0; let totalHigh = 0; let todayChecks = 0;
    const lists = { critical: [], high: [], today: [], all: [] };
    const now = new Date();
    sessions.forEach(session => {
      const isToday = session.createdAt && (() => { const d = new Date(session.createdAt.seconds * 1000); return d.getDate() === now.getDate() && d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear(); })();
      totalChecks += (session.summary?.total || 0); totalCritical += (session.summary?.critical || 0); totalHigh += (session.summary?.high || 0);
  ...
  if (drillDownType) {
    return <ResultTableWithTabs title={drillDownType === 'critical' ? '重大な疑いのある商品' : drillDownType === 'high' ? '高リスク商品' : drillDownType === 'today' ? '本日の検査商品' : '全検査商品一覧'} items={stats.lists[drillDownType]} onBack={() => setDrillDownType(null)} showDownload={true} />;
  }
         ...
         <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-3">
                {(selectedSession.status === 'aborted' || selectedSession.status === 'paused' || selectedSession.status === 'processing') && (
                  <button onClick={() => onResume(selectedSession)} className="bg-amber-500 text-white px-5 py-2 rounded-lg shadow-md hover:bg-amber-600 text-sm font-bold flex items-center gap-2 animate-pulse transition-transform active:scale-95"><PlayCircle className="w-4 h-4"/> 続きから再開 ({selectedSession.lastPage}ページ目〜)</button>
                )}
                {selectedSession.status === 'processing' && (
                  <button onClick={() => onForceStop(selectedSession.id)} className="bg-slate-100 text-slate-600 px-5 py-2 rounded-lg hover:bg-red-50 hover:text-red-600 text-sm font-bold flex items-center gap-2 transition-colors"><StopCircle className="w-4 h-4"/> 強制終了</button>
                )}
            </div>
         </div>
         <ResultTableWithTabs title={`${selectedSession.target} の履歴`} items={selectedSession.details?.map(item => ({...item, sessionUser: selectedSession.user, sessionDate: selectedSession.createdAt, source: selectedSession.target})) || []} onBack={null} currentUser={{name: selectedSession.user}} />
       </div>
    );
  }
        ...
        {Object.keys(groupedSessions).sort((a,b) => b.localeCompare(a)).map(month => (
          <div key={month} className="mb-4">
            <div onClick={() => toggleMonth(month)} className="flex items-center gap-3 cursor-pointer p-3 hover:bg-slate-50 rounded-xl select-none text-slate-700 font-bold text-lg transition-colors">
              {expandedMonths[month] ? <ChevronDown className="w-5 h-5 text-blue-500"/> : <ChevronRight className="w-5 h-5 text-slate-400"/>} <Folder className="w-5 h-5 text-blue-500 fill-blue-50"/> {month}
            </div>
                ...
                {Object.keys(groupedSessions[month]).sort((a,b) => parseInt(b)-parseInt(a)).map(day => (
                  <div key={day}>
                    <div onClick={() => toggleDay(month+day)} className="flex items-center gap-2 cursor-pointer p-2 hover:bg-slate-50 rounded-lg select-none text-sm font-bold text-slate-600 transition-colors">
                      {expandedDays[month+day] ? <ChevronDown className="w-4 h-4 text-slate-400"/> : <ChevronRight className="w-4 h-4 text-slate-300"/>} <span>{day}</span>
                    </div>
                        ...
                        {groupedSessions[month][day].map(session => (
                           <div key={session.id} onClick={() => setSelectedSession(session)} className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-xl cursor-pointer group hover:border-blue-200 hover:shadow-md transition-all">
                             <div className="flex items-center gap-4 overflow-hidden">
                                <div className={`p-2.5 rounded-xl ${session.type==='url'?'bg-blue-50 text-blue-600':'bg-emerald-50 text-emerald-600'}`}>{session.type==='url' ? <ShoppingBag className="w-5 h-5"/> : <FileText className="w-5 h-5"/>}</div>
                                <div className="min-w-0">
                                   <div className="flex items-center gap-3 mb-1"><p className="text-sm font-bold text-slate-700 truncate w-64 md:w-80">{session.target}</p><SessionStatusBadge status={session.status} /></div>
                                   <p className="text-xs text-slate-400 flex items-center gap-3"><span className="flex items-center gap-1"><User className="w-3 h-3"/> {session.user}</span><span className="flex items-center gap-1"><Calendar className="w-3 h-3"/> {new Date(session.createdAt.seconds*1000).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</span>{session.shopName && <span className="bg-slate-50 px-1.5 py-0.5 rounded text-[10px] border border-slate-100">{session.shopName}</span>}</p>
                                </div>
                             </div>
                             <div className="flex gap-3 items-center">
                               ...
                               {currentUser?.role === 'admin' && <button onClick={(e) => { e.stopPropagation(); onDelete(session.id); }} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors opacity-0 group-hover:opacity-100" title="削除"><Trash2 className="w-4 h-4" /></button>}
                             </div>
                           </div>
                        ))}
                      </div>
...
const saveSessionToFirestore = async (db, currentUser, type, target, allResults) => {
    ...
    try {
        ...
        await addDoc(collection(db, 'check_sessions'), { type, target, user: currentUser.name, createdAt: serverTimestamp(), summary, details: allResults });
...
const UrlSearchView = ({ config, db, currentUser, addToast, state, setState, stopRef, isHighSpeed, setIsHighSpeed, historySessions, onResume }) => {
  ...
  const fetchShopInfo = async () => {
    ...
    if (window.location.hostname.includes('stackblitz') || window.location.hostname.includes('webcontainer')) { alert("【注意】StackBlitzプレビューでは動作しません。Vercel環境で実行してください。"); return; }
    ...
    try {
      ...
      if (!data.count && (!data.products || data.products.length === 0)) throw new Error("商品が見つかりませんでした");
      setShopMeta({ count: data.count || 0, shopCode: data.shopCode, shopName: data.products?.[0]?.shopName || '' });
      setUrlStep('confirm');
      updateState({ status: '' });
    } catch (e) { addToast(e.message, 'error'); updateState({ status: 'エラー' }); } finally { updateState({ isProcessing: false }); }
  };
  ...
  const updateSessionStatus = async (sessId, status, lastPage, details) => {
      ...
      try {
          ...
          await updateDoc(doc(db, 'check_sessions', sessId), { status, lastPage, summary, details, updatedAt: serverTimestamp() });
  ...
  const runUrlCheckLoop = async (startP, sessId, currentResults, range) => {
      ...
      try {
          while (page <= neededPages) {
              if (stopRef.current) { await updateSessionStatus(sessId, 'paused', page - 1, totalResults); updateState({ status: '中断しました' }); break; }
              ...
              for (let i = 0; i < pageProducts.length; i += BATCH_SIZE) {
                  ...
                  updateState({ results: [...totalResults, ...pageResults], progress: ((totalResults.length + pageResults.length) / targetRange) * 100 });
      ...
      } catch (e) { addToast(e.message, 'error'); await updateSessionStatus(sessId, 'aborted', page - 1, totalResults); } finally { updateState({ isProcessing: false }); }
  };
  ...
  const handleStart = async (resumeSession = null, overrideRange = null) => {
      ...
      setUrlStep('processing'); setLiveLog([]);
      updateState({ isProcessing: true, status: '準備中...', progress: 0 });
      stopRef.current = false;

      let currentSessionId = null; let initialResults = []; let pageStart = 1;
      if (resumeSession) {
          ...
          if (db) {
              const docRef = await addDoc(collection(db, 'check_sessions'), { type: 'url', target: targetUrl, shopName: shopMeta.shopName, user: currentUser.name, createdAt: serverTimestamp(), status: 'processing', lastPage: 0, summary: { total: 0, high: 0, critical: 0 }, details: [] });
              currentSessionId = docRef.id;
          }
          initialResults = []; pageStart = 1;
          await runUrlCheckLoop(pageStart, currentSessionId, initialResults, activeRange);
      }
  };

  const handleReset = () => { setUrlStep('input'); updateState({ results: [], progress: 0, status: '' }); setShopMeta({ count: 0, shopCode: '', shopName: '' }); };
    ...
    return (
      <div className="space-y-6 animate-in fade-in w-full">
        <div className="bg-white p-10 rounded-3xl border border-slate-100 shadow-xl shadow-slate-100 max-w-4xl mx-auto text-center">
            <div className="mb-8"><div className="inline-flex p-6 bg-blue-50 rounded-full mb-6 text-blue-600 shadow-inner"><ShoppingBag className="w-16 h-16"/></div><h2 className="text-3xl font-bold text-slate-800 tracking-tight">楽天ショップ自動パトロール</h2><p className="text-slate-500 mt-3 text-lg">ショップURLを入力すると、商品数を確認してからチェックを実行できます。</p></div>
            <div className="flex flex-col md:flex-row gap-4 items-center justify-center max-w-2xl mx-auto">
                <input type="text" value={targetUrl} onChange={e => updateState({ targetUrl: e.target.value })} className="w-full px-6 py-4 border border-slate-200 rounded-2xl text-lg focus:ring-4 focus:ring-blue-100 focus:border-blue-500 outline-none shadow-sm transition-all" placeholder="https://www.rakuten.co.jp/shop-name/" />
                <button onClick={fetchShopInfo} disabled={isProcessing} className="w-full md:w-auto px-10 py-4 bg-slate-900 text-white font-bold text-lg rounded-2xl hover:bg-slate-800 shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-3 active:scale-95">{isProcessing ? <Loader2 className="w-6 h-6 animate-spin"/> : <Search className="w-6 h-6"/>} 確認</button>
            </div>
            {previousHistory && (
                 <div className="mt-8 bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center justify-center gap-3 text-amber-800"><Info className="w-5 h-5"/><span className="font-medium">過去の履歴あり: {new Date(previousHistory.createdAt.seconds*1000).toLocaleDateString()} ({previousHistory.summary?.total}件)</span>{previousHistory.status !== 'completed' && (<button onClick={() => handleStart(previousHistory)} className="ml-2 bg-white px-4 py-1.5 rounded-lg text-sm font-bold shadow-sm hover:shadow hover:text-amber-900 transition-all">続きから再開</button>)}</div>
            )}
          ...
          <div className="space-y-6 animate-in fade-in w-full">
              <div className="bg-white p-10 rounded-3xl border border-slate-100 shadow-xl shadow-slate-100 max-w-4xl mx-auto">
                  ...
                  <div className="bg-slate-50 p-8 rounded-2xl mb-10 flex flex-col md:flex-row items-center justify-between border border-slate-100 gap-6">
                      <div className="flex-1 min-w-0"><p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">TARGET SHOP</p><p className="text-2xl font-bold text-slate-800 truncate">{shopMeta.shopName || '取得中...'}</p><p className="text-sm text-slate-500 font-mono truncate opacity-70 mt-1">{targetUrl}</p></div>
                      <div className="text-right bg-white px-8 py-4 rounded-xl shadow-sm border border-slate-100"><p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">TOTAL ITEMS</p><p className="text-4xl font-bold text-blue-600 tracking-tight">{shopMeta.count.toLocaleString()} <span className="text-sm text-slate-400 font-normal">件</span></p></div>
                  </div>
                  <div className="space-y-6">
                      <p className="font-bold text-slate-700 text-lg">チェック範囲を選択してください:</p>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          <button onClick={() => { setCheckRange(30); handleStart(null, 30); }} className="group p-6 border-2 border-slate-100 hover:border-blue-500 bg-white hover:bg-blue-50/30 rounded-2xl text-left transition-all duration-300 hover:shadow-lg hover:-translate-y-1"><div className="font-bold text-xl text-slate-800 mb-2 group-hover:text-blue-700">クイック</div><div className="text-blue-600 font-bold text-3xl mb-1">30 <span className="text-sm font-normal text-slate-500">件</span></div><div className="text-xs font-bold text-slate-400 bg-slate-100 inline-block px-2 py-1 rounded">所要時間: 約30秒</div></button>
                          <button onClick={() => { setCheckRange(150); handleStart(null, 150); }} className="group p-6 border-2 border-slate-100 hover:border-blue-500 bg-white hover:bg-blue-50/30 rounded-2xl text-left transition-all duration-300 hover:shadow-lg hover:-translate-y-1"><div className="font-bold text-xl text-slate-800 mb-2 group-hover:text-blue-700">スタンダード</div><div className="text-blue-600 font-bold text-3xl mb-1">150 <span className="text-sm font-normal text-slate-500">件</span></div><div className="text-xs font-bold text-slate-400 bg-slate-100 inline-block px-2 py-1 rounded">所要時間: 約3分</div></button>
                          <button onClick={() => { setCheckRange(3000); handleStart(null, 3000); }} className="group p-6 border-2 border-slate-100 hover:border-blue-500 bg-white hover:bg-blue-50/30 rounded-2xl text-left transition-all duration-300 hover:shadow-lg hover:-translate-y-1 relative overflow-hidden">
                              ...
                              <div className="font-bold text-xl text-slate-800 mb-2 group-hover:text-blue-700">フルスキャン</div><div className="text-blue-600 font-bold text-3xl mb-1">Max <span className="text-sm font-normal text-slate-500">3000件</span></div><div className="text-xs font-bold text-slate-400 bg-slate-100 inline-block px-2 py-1 rounded">所要時間: 30分〜</div>
                          </button>
                      </div>
                  </div>
              </div>
          ...
          <div className="h-full flex flex-col animate-in fade-in space-y-6">
             <div className="bg-slate-900 text-white p-8 rounded-3xl shadow-2xl flex flex-col md:flex-row gap-8 items-center relative overflow-hidden">
                 ...
                 <div className="flex-1 w-full z-10">
                    <div className="flex items-center gap-4 mb-6"><div className="relative"><Activity className="w-10 h-10 text-emerald-400 animate-pulse"/><span className="absolute top-0 right-0 w-3 h-3 bg-emerald-400 rounded-full animate-ping"></span></div><div><h2 className="text-2xl font-bold tracking-tight">AIリアルタイム監査中</h2><p className="text-blue-200 text-sm font-mono mt-1">{status}</p></div></div>
                    <div className="w-full bg-slate-800/50 rounded-full h-4 overflow-hidden mb-3 border border-slate-700"><div className="h-full bg-gradient-to-r from-blue-500 to-emerald-400 transition-all duration-500 ease-out shadow-[0_0_15px_rgba(59,130,246,0.5)]" style={{ width: `${progress}%` }}></div></div>
                    <div className="flex justify-between text-xs font-mono text-slate-400"><span>Progress: {Math.round(progress)}%</span><span>Checked: {results.length} items</span></div>
                 </div>
                 <div className="w-full md:w-80 bg-slate-800/80 backdrop-blur rounded-2xl p-5 border border-slate-700/50 flex items-center gap-5 shadow-xl z-10">
                    {latestItem ? (
                        <><div className="relative"><img src={latestItem.imageUrl} alt="" className="w-20 h-20 object-cover rounded-xl bg-white shadow-sm" />{(latestItem.risk === '高' || latestItem.isCritical) && <div className="absolute -top-2 -right-2 bg-red-500 w-4 h-4 rounded-full animate-ping"></div>}</div><div className="min-w-0 flex-1"><p className="text-[10px] font-bold text-blue-400 uppercase tracking-wider mb-1">ANALYZING NOW</p><p className="text-sm font-bold truncate text-white mb-2">{latestItem.productName}</p><div>{(latestItem.risk === '高' || latestItem.isCritical) ? <span className="text-red-400 font-bold text-xs flex items-center gap-1 bg-red-500/10 px-2 py-1 rounded"><Siren className="w-3 h-3"/> High Risk</span> : <span className="text-emerald-400 font-bold text-xs flex items-center gap-1 bg-emerald-500/10 px-2 py-1 rounded"><CheckCircle className="w-3 h-3"/> Safe</span>}</div></div></>
             ...
             <div className="flex-1 min-h-0 bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden flex flex-col">
                 <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center"><h3 className="font-bold text-slate-700 flex items-center gap-2"><Terminal className="w-4 h-4"/> 検出ログ</h3><button onClick={() => stopRef.current = true} className="text-xs font-bold text-red-500 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-colors border border-transparent hover:border-red-100">中断して結果を見る</button></div>
                 <div className="flex-1 overflow-hidden"><ResultTableWithTabs items={results} currentUser={currentUser} title="" showDownload={false} /></div>
             </div>
          </div>
      );
  }
  ...
  if (urlStep === 'result') {
      return <ResultTableWithTabs items={results} currentUser={currentUser} title="検索結果一覧" onBack={handleReset} onReset={handleReset} />;
  }
...
const CsvSearchView = ({ config, db, currentUser, addToast, state, setState, stopRef, isHighSpeed, setIsHighSpeed }) => {
  ...
  if (!isProcessing && results.length > 0) {
      return <ResultTableWithTabs items={results} currentUser={currentUser} title="CSV検査結果" onBack={() => updateState({ results: [], files: [] })} />;
  }
